...
<div class="min-h-screen flex items-center justify-center p-6" style="background: #0f172a; background: linear-gradient(to bottom, #0f172a, #1e293b, #0f172a);">
	<div class="w-full max-w-sm">
		...
		<div class="rounded-md p-6 border" style="background-color: rgba(30, 41, 59, 0.5); border-color: rgba(255, 255, 255, 0.1);">
			{#if !cardToken}
			...
			{:else}
				<form onsubmit={(e) => { e.preventDefault(); login(); }} class="space-y-5">
					...
					<button
						type="submit"
						disabled={loading}
						class="w-full text-white py-3 rounded-md font-medium transition-all disabled:opacity-50 shadow-lg"
			...
			{#if error}
				<div class="mt-4 p-3 text-red-400 rounded-md text-sm" style="background-color: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2);">
					{error}
				</div>
			{/if}
		</div>
